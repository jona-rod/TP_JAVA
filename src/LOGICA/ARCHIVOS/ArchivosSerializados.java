...
public abstract class ArchivosSerializados {
    ...
    private static void cargarZonas(TreeMap<String, Zona> conjuntoZonas, ArrayList<Zona> listadoZonas, ArrayList<Stand> listadoStands) {
        try (ObjectInputStream cargaSerializados = new ObjectInputStream(new FileInputStream("src/LOGICA/ARCHIVOS/zonasSerializado.dat"))) {
            Zona zona;
            while (true) {
    ...
    private static void guardarZonas(TreeMap<String, Zona> conjuntoZonas) {
        try (ObjectOutputStream serializa = new ObjectOutputStream(new FileOutputStream("src/LOGICA/ARCHIVOS/zonasSerializado.dat"))) {
    ...
    private static void cargarPersonas(TreeMap<String, Persona> listadoPersonas) {
        try (ObjectInputStream cargaSerializados = new ObjectInputStream(new FileInputStream("src/LOGICA/ARCHIVOS/personasSerializado.dat"))) {
            Persona persona;
            while (true) {
    ...
    private static void guardarPersonas(TreeMap<String, Persona> listadoPersonas) {
        try (ObjectOutputStream serializa = new ObjectOutputStream(new FileOutputStream("src/LOGICA/ARCHIVOS/personasSerializado.dat"))) {
    ...
    public static boolean archivosNoExisten() {
        File fZonas = new File("src/LOGICA/ARCHIVOS/zonasSerializado.dat");
        File fPersonas = new File("src/LOGICA/ARCHIVOS/personasSerializado.dat");
